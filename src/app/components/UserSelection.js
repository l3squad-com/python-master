...
export default function UserSelection({ selectUser }) {
  ...
  useEffect(() => {
    ...
    const storedResults = localStorage.getItem('python_quizResults');
    if (storedResults) {
  ...
  const addUser = (name) => {
    ...
    localStorage.setItem('python_quizUserId', newUserId);
    ...
    localStorage.setItem('python_quizResults', JSON.stringify(updatedUsers));
