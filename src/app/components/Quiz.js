...
export default function Quiz({ quizType, user, resetQuiz }) {
  ...
  const finishQuiz = () => {
    ...
    const currentUserId = localStorage.getItem('python_quizUserId');

    if (currentUserId) {
      // Get existing quiz results from local storage
      const storedResults = JSON.parse(localStorage.getItem('python_quizResults') || '[]');
      ...
      localStorage.setItem('python_quizResults', JSON.stringify(updatedResults));
    }
  };
